...
export default function BlogSection({
  ...
  return (
    <section aria-labelledby="blog-heading" className={cn("w-full max-w-full", className)}>
      <header className="mb-6 sm:mb-8">
        <div className="flex items-center gap-2 text-foreground">
          ...
          <h2 id="blog-heading" className="font-heading text-lg sm:text-xl md:text-2xl">
            {title}
          </h2>
        </div>
        {description ? (
      ...
      <div className="grid grid-cols-1 gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {posts.map((post) => {
          ...
          const Content = (
            <Card className="group h-full rounded-[var(--radius)] border border-border bg-card transition-shadow duration-200 hover:shadow-sm">
              <article className="flex h-full flex-col">
                ...
                <CardContent className="pb-0">
                  <p className="text-sm text-foreground/80 break-words">{post.excerpt}</p>
                </CardContent>
